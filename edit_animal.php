...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($image['error'] === UPLOAD_ERR_OK) {
        ...
        $image_path = 'images/animals/' . $image_name;

        // Déplacer le fichier téléchargé dans le répertoire souhaité
        if (move_uploaded_file($image['tmp_name'], $image_path)) {
            // Supprimer l'ancienne image si elle existe
            if (isset($animal['image']) && file_exists('images/animals/' . $animal['image'])) {
                unlink('images/animals/' . $animal['image']);
            }
        } else {
...
<body>
    ...
    <form method="POST" action="edit_animal.php?id=<?= $animal_id ?>" enctype="multipart/form-data">
        ...
        <?php if (isset($animal['image']) && $animal['image']): ?>
            <p>
                <img src="images/animals/<?= htmlspecialchars($animal['image']) ?>" alt="<?= htmlspecialchars($animal['name']) ?>" width="150">
                <br>
